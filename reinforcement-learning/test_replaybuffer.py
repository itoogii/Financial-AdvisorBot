...
observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(60, 5), dtype=np.float32)
action_space = gym.spaces.Discrete(3, dtype=np.int8)

# ReplayBuffer object with size=10
test_rb = ReplayBuffer(
    10,
    observation_space,
    action_space,
